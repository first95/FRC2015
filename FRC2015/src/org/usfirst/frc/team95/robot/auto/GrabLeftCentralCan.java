...
public class GrabLeftCentralCan extends PureSequentialMove {
	...
	public GrabLeftCentralCan(Robot robot) {
		// AutoMove[] stuff = { new PlainMotorMove(robot.armMotors, -0.5, 0.5),
		// new GoBackward(robot) };
		AutoMove[] moves = { /* new MoveArmTo(robot, 1.497) */
				new PlainMotorMove(robot.armMotors, 0.5, 1.5),
				new GoForward(robot, 0.5, 1.0),
				new Pistons(robot, true),
				new PlainMotorMove(robot.armMotors, 0, 0.002),
				/* new SimultaniousMove(stuff) */new PlainMotorMove(
						robot.armMotors, -0.5, 2.5), new GoBackward(robot),
